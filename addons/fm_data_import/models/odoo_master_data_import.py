...
class OdooMasterDataImportConfig(models.Model):
    _name = 'odoo.master.data.config'
    _description = 'Odoo Master Data Import Configuration'
    ...
    last_import = fields.Datetime(
        ...
        readonly=True,
        help='Date and time of the last successful import'
    )
    import_log = fields.Text(
        string='Import Log',
        readonly=True,
        help='Log of the latest import operation'
    )
    state = fields.Selection(
        [('draft', 'Draft'), ('configured', 'Configured'), ('imported', 'Imported')],
        string='State',
        default='draft',
        readonly=True
    )
    ...
    def action_import_master_data(self):
        """Execute the master data import."""
        self.ensure_one()
        ...
        try:
            ...
            self.write({'import_log': error_msg})
            raise UserError(error_msg)
